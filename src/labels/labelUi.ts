...
export class LabelUi {
      ...
      for (const pos of this.labelManager.labels) {
        ...
        for (const v of [face.a, face.b, face.c]) {
          ...
          if (idx != null) {
            ...
            if (this.onActiveLabelChangeHandler != null) { this.onActiveLabelChangeHandler(pos); }
    ...
    public loadGivenLabels (labels: Label[]): void {
      ...
      labels.forEach(label => {
        ...
        this.listContainer.append(element);
        if (this.visible) { this.labelManager.renderer.setColorForVertices(label.vertices, label.color); }
    ...
    private setActiveLabel (event: Event): void {
      ...
      if (label == null) { throw `Could not find label with id ${this.activeLabel ?? 0}`; }

      if (target.checked) {
        this.activeLabel = label.id;
        this.lastClickTarget = label.id;
        if (this.onActiveLabelChangeHandler != null) { this.onActiveLabelChangeHandler(label); }
      }
    }
